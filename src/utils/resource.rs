...
impl FromStr for ResourceValue {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.ends_with('%') {
            ...
            let percentage = percentage_str.parse::<u8>()
...
mod tests {
    ...
    fn test_from_str_absolute_memory() {
        assert_eq!(ResourceValue::from_str("4G").unwrap(), ResourceValue::Absolute(4 * 1024 * 1024 * 1024));
        assert_eq!(ResourceValue::from_str("512M").unwrap(), ResourceValue::Absolute(512 * 1024 * 1024));
        assert_eq!(ResourceValue::from_str("1024K").unwrap(), ResourceValue::Absolute(1024 * 1024));
        assert_eq!(ResourceValue::from_str("100").unwrap(), ResourceValue::Absolute(100));
    }

    #[test]
    fn test_from_str_percentage() {
        assert_eq!(ResourceValue::from_str("50%").unwrap(), ResourceValue::Percentage(50));
        assert_eq!(ResourceValue::from_str("100%").unwrap(), ResourceValue::Percentage(100));
    }
    ...
    fn test_to_string_absolute() {
        assert_eq!(ResourceValue::Absolute(4 * 1024 * 1024 * 1024).to_string(), (4 * 1024 * 1024 * 1024).to_string());
